...
def main(args):
    ...
    log_base_path = os.path.join(args.mylogs, args.name)
    args.log_path = None
    ...
    if args.if_finutune_GPT == 0:
        ...
        assert args.if_lora == 1
        for param in model.gpt.named_parameters():
            if 'lora_' in param[0] or 'norm' in param[0]:
    ...
    if is_master(args):
        ...
        logging.info("Params:")
        params_file = os.path.join(args.mylogs, args.name, "params.txt")
    ...
    # initialize datasets
    if args.train_data != '':
        data = get_data(args, (None, None), epoch=start_epoch, tokenizer=model.module)
    else:
    ...
    args.save_logs = args.mylogs and args.mylogs.lower() != 'none' and is_master(args)
    writer = None
    ...
    if is_master(args):
        for i, param in enumerate(model.module.named_parameters()):
            print(i, param[0], param[1].requires_grad)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if if_load and 'val' in data and is_master(args):
            evaluate(model.module, data, start_epoch, args, writer)
            if_load = False

        train_one_epoch(model, data, epoch, optimizer, scaler, scheduler, args, tb_writer=writer)
        completed_epoch = epoch + 1

        if 'val' in data and is_master(args):
            _, score = evaluate(model.module, data, completed_epoch, args, writer)

        # Saving checkpoints.
        if args.save_logs and score > max_score:
            checkpoint_save = OrderedDict()
            for k, v in model.state_dict().items():
                if args.if_lora == 1:
                    if not ('gpt.' in k and '.lora_' not in k and 'norm' not in k):
                        checkpoint_save[k] = v
                else:
                    if 'gpt.' not in k:
                        checkpoint_save[k] = v
            ...
            if args.delete_previous_checkpoint:
                previous_checkpoint = os.path.join(args.checkpoint_path, f"epoch_{last_checkpoint}.pt")
                if os.path.exists(previous_checkpoint):
                    os.remove(previous_checkpoint)

            last_checkpoint = completed_epoch
            max_score = score
