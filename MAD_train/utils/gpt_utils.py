...
@torch.no_grad()
def generate_beam(model, beam_size: int = 5, attention_mask=None, charactor_images=None, video_embeds=None, text_input=None, idx=None, video_idx=None,
                  entry_length=67, temperature=1., past_key_values=None, media=None,
                  repetition_penalty=1.2,
                  no_repeat_ngram_size=3,
                  history_tokens=None,):

    model.eval()
    stop_token_index = model.tokenizer.eos_token_id
    tokens = None
    scores = None
    ...
    for i in range(entry_length):
        if i == 0:
            if media is not None:
                outputs, generated, vis_x = model(video_embeds=video_embeds, text_input=text_input, charactor_images=charactor_images, text_mask=attention_mask, past_key_values=past_key_values, media=media, if_train=False, idx=idx, video_idx=video_idx)
                # generated = generated.unsqueeze(0)
            else:
                outputs, generated, vis_x = model(video_embeds=video_embeds, text_input=text_input, charactor_images=charactor_images, text_mask=attention_mask, past_key_values=past_key_values, if_train=False, idx=idx, video_idx=video_idx)
                # generated = generated.unsqueeze(0)
        else:
            if model.if_special_prompt == 1:
                outputs = model.gpt(
                ...
                outputs = model.gpt(
                    ...
                    return_dict=True,
                    labels=None,
                    lm_heads_new=[model.image_output_token_embedding],
                    video_length=None,
                    past_key_values=past_key_values,
                    if_train=False,
                    vis_x=vis_x)
        ...
        next_token_embed = model.get_embed(next_tokens.squeeze()).view(generated.shape[0], 1, -1)
        if past_key_values is not None:
    ...
    tokens_temp = tokens.masked_fill(tokens > 32000, 0)
    output_list = tokens_temp.cpu().numpy()
    output_texts = [model.tokenizer.decode(output[:int(length)], skip_special_tokens=True) for output, length in zip(output_list, seq_lengths)]
    order = scores.argsort(descending=True)
    output_texts = [output_texts[i] for i in order]
    return [output_texts[0]]  # return the max?
