...
class VideoCaptionModel(nn.Module):
    ...
    def forward(self, charactor_images=None, video_embeds=None, text_input=None, text_mask=None, past_key_values=None, text_output=None, output_mask=None, if_train=True, idx=None, video_idx=None, ad_start_ids=None):
        """
        Process:
        ...
        if if_train:
            # mask [PAD]
            assert ad_start_ids is not None
            assert len(ad_start_ids) == B

            targets = sequence_input.masked_fill(
                sequence_input == self.pad_token_id, -100
            )
            # mask tokens before ad_start
            batch_len = torch.arange(sequence_input.shape[1]).unsqueeze(0).to(sequence_input.device)
            ad_start_ids = torch.tensor(ad_start_ids).view(-1, 1).to(sequence_input.device)
            targets = targets.masked_fill(
                batch_len <= ad_start_ids, -100
            )
        ...
        if self.if_regression and if_train:
            regress_label_mask = (sequence_input == self.video_token_id).to(video_embeds.device)
            regress_labels = text_embeds[regress_label_mask]
            regress_mask = (sequence_input == self.video_token_id).to(video_embeds.device)
