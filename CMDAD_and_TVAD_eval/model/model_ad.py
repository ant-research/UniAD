...
from .model_tfm import PerceiverEncoder, GatedCrossAttentionBlock, TFMEncoder, PerceiverResampler
...
from video_llama.models.video_llama_VL import VideoLLAMA_VL
...
class LlamaAttentionForReg(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None, r=8, lora_alpha=16, lora_layers=None):
        super().__init__()
        self.config = config
        if self.config.bos_token_id == 128000:
            ...
            self.q_proj = lora.Linear(self.hidden_size, self.num_heads * self.head_dim, r=16, lora_alpha=16, lora_dropout=0.05, bias=config.attention_bias)
            self.k_proj = lora.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, r=16, lora_alpha=16, lora_dropout=0.05, bias=config.attention_bias)
            self.v_proj = lora.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, r=16, lora_alpha=16, lora_dropout=0.05, bias=config.attention_bias)
            self.o_proj = lora.Linear(self.num_heads * self.head_dim, self.hidden_size, r=16, lora_alpha=16, lora_dropout=0.05, bias=config.attention_bias)
...
class LlamaDecoderLayerForReg(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int, r=8, lora_alpha=16, lora_layers=None):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = LlamaAttentionForReg(config=config, layer_idx=layer_idx, r=r, lora_alpha=lora_alpha, lora_layers=lora_layers)
...
class LlamaModelForReg(LlamaModel):
    def __init__(self, config: LlamaConfig, r=8, lora_alpha=16, lora_layers=None):
        ...
        self.layers = nn.ModuleList([LlamaDecoderLayerForReg(config, layer_idx, r=r, lora_alpha=lora_alpha, lora_layers=lora_layers) for layer_idx in range(config.num_hidden_layers)])
        self.post_init()

class LlamaModel_flamingo(LlamaModel):
    def forward(
        self,
        ...
        for idx, (decoder_layer, gated_cross_attn_layer) in enumerate(zip(self.layers, gated_cross_attn_layers)):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if gated_cross_attn_layer is not None:
                    hidden_states = torch.utils.checkpoint.checkpoint(
                        create_custom_forward(gated_cross_attn_layer),
                        hidden_states,
                        vis_x,
                ...
                if gated_cross_attn_layer is not None:
                    hidden_states = gated_cross_attn_layer(
                        hidden_states,
                        vis_x,
                        media_locations=media_locations,
                    )
...
class LlamaModel_flamingo_lora(LlamaModel):
    def __init__(self, config: LlamaConfig, r=8, lora_alpha=16, lora_layers=None):
        super().__init__(config)
...
class LlamaForReg_gated(transformers.LlamaForCausalLM):
    def __init__(self, config, **kwargs):
        super().__init__(config)
        self.model = LlamaModel_flamingo(config)
        self.vocab_size = config.vocab_size
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
        cross_attn_every_n_layers = kwargs.pop("cross_attn_every_n_layers", None)

        self.gated_cross_attn_layers = nn.ModuleList(
            [
                GatedCrossAttentionBlock(
                    dim=config.hidden_size, dim_visual=config.hidden_size, dim_head=128, heads=32, only_attend_immediate_media=False
                )
...
class LlamaForReg_gated_lora(transformers.LlamaForCausalLM):
    def __init__(self, config, r=8, lora_alpha=16, lora_layers=None, **kwargs):
        super().__init__(config)
        self.model = LlamaModel_flamingo_lora(config, r=r, lora_alpha=lora_alpha, lora_layers=lora_layers)
        self.vocab_size = config.vocab_size
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
        cross_attn_every_n_layers = kwargs.pop("cross_attn_every_n_layers", None)
...
class VideoCaptionModel(nn.Module):
    def __init__(self,
        ...
        if if_only_flamingo == 2:
            ...
            if args.if_lora == 0:
                self.gpt = LlamaForReg_gated.from_pretrained(LLM_path, cross_attn_every_n_layers=args.cross_attn_every_n_layers)
            else:
                self.gpt = LlamaForReg_gated_lora.from_pretrained(LLM_path, cross_attn_every_n_layers=args.cross_attn_every_n_layers)
        self.gpt_embedding_size = self.gpt.model.embed_tokens.weight.shape[1]
        if 'LLaMA3_1' in LLM_path:
        ...
        self.video_with_image = args.video_with_image
        ...
        if if_special_prompt == 1:    
            ...
            self.context_ad_token_id = self.num_embeddings + 2
            
            self.pad_token_id = self.num_embeddings + 3
            ...
            self.context_num = 1
            self.context_input_token_embedding = nn.Embedding(self.context_num, embedding_dim=self.gpt_embedding_size)
            self.context_input_token_embedding.weight.data = input_init.repeat(self.context_num, 1)
            self.context_output_token_embedding = nn.Linear(self.gpt_embedding_size, self.context_num, bias=False)
            self.context_output_token_embedding.weight = self.context_input_token_embedding.weight
        
        ### visual ###
        if if_only_flamingo != 1:
            self.perceiver = VideoLLAMA_VL()

            if self.char_prompt_type < 2 and self.if_share_img_video == 0:
                
                self.project_img = nn.Linear(768, self.gpt_embedding_size, dtype=torch.float32)
                self.project_img.weight = copy.deepcopy(self.perceiver.llama_proj.weight)
                self.project_img.bias = copy.deepcopy(self.perceiver.llama_proj.bias)

        if if_only_flamingo != 2:
            self.perceiver_all = PerceiverResampler(
                    dim=self.gpt_embedding_size,
                    depth=num_layers,
                    dim_head=128,
                    heads=self.gpt_embedding_size//128,
    ...
    def get_embed(self, text_input=None,):
        ...
        tokens_video = sequence_input.clone()
        tokens_char = sequence_input.clone()
        tokens_context = sequence_input.clone()
        ...
        mask_context = (tokens_char == self.context_ad_token_id)
        tokens_context -= self.context_ad_token_id
        tokens_context[~mask_context] = 0
        context_embeds = self.context_input_token_embedding(tokens_context)

        text_embeds[mask_video] = video_embeds_[mask_video]
        text_embeds[mask_img] = img_embeds[mask_img]
        text_embeds[mask_context] = context_embeds[mask_context]
    ...
    def forward(self, charactor_images=None, video_embeds=None, text_input=None, text_mask=None, past_key_values=None, text_output=None, output_mask=None, if_train=False, idx=None, video_idx=None, ad_start_ids=None):
        ...
        if self.if_only_flamingo != 1:
            if self.video_with_image == 0:
                perceiver_embeds = self.perceiver(video_embeds)
            else:
                for i in range(len(video_embeds)):
                    current_video = video_embeds[i]
                    if charactor_images is not None:
                        ...
                        # assert self.char_prompt_type > 1
                    current_video = current_video.unsqueeze(0)
                    perceiver_embeds_ = self.project(current_video)
                    ...
                    if gated_embeds is None:
                        gated_embeds = perceiver_embeds
                    else:
                        gated_embeds = torch.cat((gated_embeds, perceiver_embeds), dim=0)
            if self.video_with_image == 0:
                perceiver_embeds = perceiver_embeds.reshape(-1, perceiver_embeds.shape[-1]).float()
                text_embeds[all_video_indices] = perceiver_embeds
            else:
                gated_embeds = gated_embeds.reshape(-1, gated_embeds.shape[-1]).float()
                text_embeds[all_video_indices] = gated_embeds
            ...
            if charactor_images is not None:
                ...
                if self.if_share_img_video == 0:
                    assert all_img_indices.sum() == 0
                    nothing = self.project_img(torch.zeros((video_embeds.shape[0], video_embeds.shape[2], video_embeds.shape[3])).to(video_embeds.device))
