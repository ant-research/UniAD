...
class video_sample(object):
    ...
    def __call__(self, video: Any) -> Any:
        ...
        if frames_num < self.num_frame:
            zero_pad = np.zeros((self.num_frame - frames_num, video.shape[-2], video.shape[-1]), dtype=video.dtype)
...
class JsonDataset_EVAL(Dataset):
    def __init__(self, eval_data, movie_feature_path, char_feature_path, model, movie_id, if_special_prompt, if_train_drop, char_prompt_type, previous_video_num, if_video_split, char_therehold):
        self.data = eval_data
        if movie_id == 'cmdad':
            self.video = h5py.File('/home/whl/UniAD_open_source/CMDAD/VideoLLaMa_CMD_eval_fp16.h5','r')
        elif movie_id == 'tvad':
            self.video = h5py.File('/home/whl/UniAD_open_source/CMDAD/TV_eval_videollama.h5','r')
        else:
            assert 0
        ...
        self.char_therehold = char_therehold
    ...
    def __getitem__(self, idx):
        ...
        movie_id = self.data[idx]['key_name']
        ad = self.data[idx]['ad']

        if 'prob' in self.data[idx].keys():
            char = self.data[idx]['prob']
        else:
            char = []
        # if self.is_train:
        try:
            movie = self.video[movie_id + '_' + str(start) + '_' + str(end)][:]
        ...
        if len(movie) >= self.model.video_length:
            ids = np.linspace(0, len(movie) - 1, num=self.model.video_length, dtype=int)
            video = movie[ids]

        else:
            zero_video = np.zeros((self.model.video_length - len(movie), movie.shape[-2], movie.shape[-1]), dtype=movie.dtype)
            video = np.concatenate((movie, zero_video), axis=0)

        if self.previous_video_num > 0:
            if 'content' not in self.data[idx].keys():
                return [], None, None, None
            for previous in range(self.previous_video_num):
                times = self.previous_video_num - previous
                times -= 1
                assert times == 0
                previous_start = self.data[idx]['content']['start']
                previous_end = self.data[idx]['content']['end']
                if previous_start < 0 or previous_end < 0:
                    return [], None, None, None
                
                try:
                    previous_movie = self.video[movie_id + '_' + str(previous_start) + '_' + str(previous_end)][:]
                except KeyError as e:
                    return [], None, None, None

                if len(previous_movie) >= self.model.video_length:
                    ...
                    previous_zero_video = np.zeros((self.model.video_length - len(previous_movie), previous_movie.shape[-2], previous_movie.shape[-1]), dtype=previous_movie.dtype)
                    previous_video = np.concatenate((previous_movie, previous_zero_video), axis=0)
        ...
        for i, c in enumerate(char):
            id = c['id']
            name = c['name']
            if 'char_in_ad' in c.keys():
                role = c['char_in_ad']
            else:
                continue

            if role in ['He','She','The','They','Them','It','Its','As','And','But','Or','How','This','That','In']:
                continue

            if id + '.jpg' not in self.char_feature.keys():
                continue
            
            if self.char_prompt_type <= 1:
                char_image_fea = self.char_feature[id + '.jpg']
                ...
                for _ in range(self.model.char_length):
        ...
        full_text.append(self.model.context_ad_token_id)
...
def get_json_dataset(args, is_train, model, eval_data=None, movie_id=None):
    dataset = JsonDataset_EVAL(
        ...
        char_therehold=args.char_therehold
    )
